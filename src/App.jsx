...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `https://boiled-industrious-contraption.glitch.me/${inputUrl}`
      );
      if (response.ok) {
        const contentType = response.headers.get("content-type");
        setOutput("Success! The format type is: " + contentType); // Set output format to the content type
      } else {
        throw new Error("Failed to fetch data");
      }
    } catch (error) {
      console.error("Error fetching data:", error.message);
      setOutput("Error while fetching!");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex items-center w-screen h-screen justify-center">
      <Header />
      <div className="bg-white w-1/2 h-full flex flex-col justify-center">
        <section className="pt-8 px-12">
          <div className="max-w-screen-xl mx-auto py-28 gap-12 text-gray-600 md:px-8">
            <div className="space-y-5 max-w-4xl mx-auto text-left">
              <h2 className="text-4xl rounded-xl text-gray-800 font-extrabold mx-auto md:text-7xl">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#000000] to-[#8b8b8b]">
                  No more <br /> CORS errors!
                </span>
              </h2>
              <p className="max-w-2xl font-mono pt-4 text-md font-semibold text-left">
                CorsAid facilitates seamless cross-origin resource sharing by
                acting as a secure proxy, allowing web applications to access
                resources from different domains without encountering CORS
                restrictions.
              </p>
              <div className="items-center justify-left pt-6 gap-x-3 space-y-3 sm:flex sm:space-y-0">
                <button
                  href="javascript:void(0)"
                  className="block py-3 px-8 text-white font-medium bg-black hover:bg-zinc-800 rounded-xl duration-150rounded-lg shadow-lg hover:shadow-none"
                >
                  Try it out
                </button>
              </div>
      ...
      <div className="bg-black w-1/2 h-full flex items-center justify-center ">
        <div className=" outline outline-2 w-[60%] h-[70%] mt-16 ml-16 rounded-xl flex flex-col justify-evenly gap-8 items-center">
          <div className="w-[80%] flex gap-8 flex-col">
            ...
            <div className="w-full flex flex-col items-center">
              <div className="h-auto w-full flex flex-row items-center justify-center gap-2">
                <div className="flex flex-row items-center w-full text-zinc-200 py-4 px-10">
                  <p className="text-center font-mono">
                    `https://boiled-industrious-contraption.glitch.me/URL`
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="w-full flex gap-8 flex-col">
            ...
            <div className="w-full flex flex-col items-center">
              <div className="h-auto w-[70%] flex flex-row gap-2">
                <div className="flex flex-row items-center w-full text-black bg-white rounded-md">
                  <div className="px-3 text-black rounded-l-md ">GET</div>
                  <input
                    type="text"
                    ...
                    id="website-url"
                    style={{ color: "#000" }} // Change the color value as needed
                    className="w-fullp-2.5 bg-transparent text-black outline-none"
                  />
                </div>
                <button
                  disabled={loading}
                  onClick={fetchData}
                  className="block py-3 px-5 text-black font-medium bg-white disabled:pointer-events-none hover:bg-zinc-400 rounded-lg duration-150 hover:shadow-none"
                >
                  {loading ? "Fetching..." : "Test"}
                </button>
              </div>
              <p className="mt-2 py-0 my-0">{output}</p>
            </div>
          </div>
        </div>
      </div>
      <div className=" bg-white text-black w-48 flex items-center justify-center h-48 top-32 absolute rounded-full">
        <Svg />
      </div>
    </div>
  );
